...
internal class ResponseInfoResponseHandler constructor(
    ...
    override fun onSuccess(statusCode: Int, headers: Array<out Header>?, response: JSONObject?) {
        ...
        val responseJSONObject = json.getObjectOrNull("response")

        if (responseJSONObject == null) {
            onFailure(NullPointerException())
        } else {
            val attachments = mutableListOf<Media>()
            val attachmentsJSONArray = responseJSONObject.getArrayOrNull("attachments")
            ...
            val formJSONObject = responseJSONObject.getObjectOrNull("form")
