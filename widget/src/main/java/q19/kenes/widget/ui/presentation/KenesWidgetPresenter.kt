...
import q19.kenes.widget.data.network.file.DownloadResult
import q19.kenes.widget.data.network.file.downloadFile
import q19.kenes.widget.data.network.http.IceServersTask
import q19.kenes.widget.data.network.http.WidgetConfigsTask
...
//        }
                ...
                if (configs.preferences.isAudioCallEnabled || configs.preferences.isVideoCallEnabled) {
                    if (configs.preferences.isCallAgentsScoped) {
                        if (!configs.calls.isNullOrEmpty()) {
                            ...
                            view?.showNavButton(BottomNavigation.CALL)
                        }
                    } else {
                    ...
                    view?.hideNavButton(BottomNavigation.CALL)
                }
    ...
    fun onCancelPendingCallClicked(bottomNavigation: BottomNavigation) {
        ...
        viewState = when (bottomNavigation) {
            ...
            BottomNavigation.CALL -> ViewState.CallAgentCall
...
//                    debug(TAG, "onTextMessage: chatFooterAdapter?.showGoToHomeButton()")
    ...
    override fun onCategories(categories: List<kz.q19.domain.model.message.Category>) {
